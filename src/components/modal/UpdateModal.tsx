...
// TODO : 해당 게시글 아이디값을 불러오고 쏴준다 . set함수에 다 싹 쏴준다.그리고 그걸 updateDoc으로 업데이트 시켜준다.
export default function UpdateModal(){
  ...
  const {modalBubbling, setUpdateModal, postId} = useModal()
...
const fetchUpdate = async () =>{
  ...
  const onSubmit = async(e:React.FormEvent<HTMLFormElement>)=>{
    ...
    try{
      ...
      if(file){
        const locationRef = ref(storage, `tweets/${user.uid}/${postId}`);
                        ...
                        <div className="post__info">
                            <div className="post__first">
                                <div className="post__number">
                                    ...
                                    <input type="number"  placeholder="팀원 수" onChange={onNumberChange} value={number}/>
                                </div>
                                <div className="post__date">
                                    <label htmlFor="">모집마감일</label>
                                    <input onChange={onDateChange} type="date" value={date} />
                                </div>
                            </div>
